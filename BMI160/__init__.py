...
class BMI160:
    ...
    def _sign_extend(val, orig):
        (((val) & (1 << ((orig) - 1))) ? (val | (((1 << (1 + (sizeof(val) << 3) - (orig))) - 1) << (orig))) : val)
    ...
    def getDeviceID():
        return reg_read(register.CHIP_ID)
    ...
    # @see BMI160GyroRate
    def getGyroRate():
        return reg_read_bits(registers.GYRO_CONF, definitions.GYRO_RATE_SEL_BIT, definitions.GYRO_RATE_SEL_LEN)
    ...
    # @see registers.GYRO_CONF
    def setGyroRate(rate):
        reg_write_bits(registers.GYRO_CONF, rate, definitions.GYRO_RATE_SEL_BIT, definitions.GYRO_RATE_SEL_LEN)
    ...
    # @see BMI160AccelRate
    def getAccelRate():
        return reg_read_bits(registers.ACCEL_CONF, definitions.ACCEL_RATE_SEL_BIT, definitions.ACCEL_RATE_SEL_LEN)

    ## Set accelerometer output data rate.
    # @param rate New output data rate
    # @see getAccelRate()
    # @see registers.ACCEL_CONF
    def setAccelRate(rate):
        reg_write_bits(registers.ACCEL_CONF, rate, definitions.ACCEL_RATE_SEL_BIT, definitions.ACCEL_RATE_SEL_LEN)
    ...
    # @see BMI160DLPFMode
    def getGyroDLPFMode():
        return reg_read_bits(registers.GYRO_CONF, definitions.GYRO_DLPF_SEL_BIT, definitions.GYRO_DLPF_SEL_LEN)

    ## Set gyroscope digital low-pass filter configuration.
    # @param mode New DLFP configuration setting
    # @see getGyroDLPFMode()
    def setGyroDLPFMode(mode):
        return reg_write_bits(registers.GYRO_CONF, mode, definitions.GYRO_DLPF_SEL_BIT, definitions.GYRO_DLPF_SEL_LEN)
    ...
    # @see registers.GYRO_CONF
    # @see BMI160DLPFMode
    def getAccelDLPFMode():
        return reg_read_bits(registers.ACCEL_CONF, definitions.ACCEL_DLPF_SEL_BIT, definitions.ACCEL_DLPF_SEL_LEN)

    ## Set accelerometer digital low-pass filter configuration.
    # @param mode New DLFP configuration setting
    # @see getAccelDLPFMode()
    def setAccelDLPFMode(mode):
        return reg_write_bits(registers.ACCEL_CONF, mode, definitions.ACCEL_DLPF_SEL_BIT, definitions.ACCEL_DLPF_SEL_LEN)
    ...
    # @see BMI160GyroRange
    def getFullScaleGyroRange():
        return reg_read_bits(registers.GYRO_RANGE, definitions.GYRO_RANGE_SEL_BIT, definitions.GYRO_RANGE_SEL_LEN)

    ## Set full-scale gyroscope range.
    # @param range New full-scale gyroscope range value
    # @see getFullScaleGyroRange()
    def setFullScaleGyroRange(range, real):
        reg_write_bits(registers.GYRO_RANGE, range, definitions.GYRO_RANGE_SEL_BIT, definitions.GYRO_RANGE_SEL_LEN)
        gyro_range = real
    ...
    def getFullScaleAccelRange():
        return reg_read_bits(registers.ACCEL_RANGE, definitions.ACCEL_RANGE_SEL_BIT, definitions.ACCEL_RANGE_SEL_LEN)

    ## Set full-scale accelerometer range.
    # @param range New full-scale accelerometer range setting
    # @see getFullScaleAccelRange()
    # @see BMI160AccelRange
    def setFullScaleAccelRange(range, real):
        reg_write_bits(registers.ACCEL_RANGE, range, definitions.ACCEL_RANGE_SEL_BIT, definitions.ACCEL_RANGE_SEL_LEN)
        accel_range = real
    ...
    def getAccelOffsetEnabled():
        return 0 != (reg_read_bits(registers.OFFSET_6, definitions.ACC_OFFSET_EN, 1))

    ## Set accelerometer offset compensation enabled value.
    # @see getXAccelOffset()
    # @see registers.OFFSET_6
    def setAccelOffsetEnabled(enabled):
        reg_write_bits(registers.OFFSET_6, 1 if enabled else 0, definitions.ACC_OFFSET_EN, 1)
    ...
    def autoCalibrateXAccelOffset(target):
        ...
        reg_write(registers.FOC_CONF, foc_conf)
        reg_write(registers.CMD, commands.START_FOC)
        while (not (reg_read_bits(registers.STATUS, definitions.STATUS_FOC_RDY, 1))):
            sleep_ms(1)
    ...
    def getXAccelOffset():
        return reg_read(registers.OFFSET_0)
    ...
    # @see getXAccelOffset()
    # @see registers.OFFSET_0
    def setXAccelOffset(offset):
        reg_write(registers.OFFSET_0, offset);
    ...
    def getYAccelOffset():
        return reg_read(registers.OFFSET_1)
    ...
    # @see getYAccelOffset()
    # @see registers.OFFSET_1
    def setYAccelOffset(offset):
        reg_write(registers.OFFSET_1, offset);
    ...
    def getZAccelOffset():
        return reg_read(registers.OFFSET_2)
    ...
    # @see getZAccelOffset()
    # @see registers.OFFSET_2
    def setZAccelOffset(offset):
        reg_write(registers.OFFSET_2, offset);
    ...
    def getGyroOffsetEnabled():
        return 0 != (reg_read_bits(registers.OFFSET_6, definitions.GYR_OFFSET_EN, 1))

    ## Set gyroscope offset compensation enabled value.
    # @see getXGyroOffset()
    # @see registers.OFFSET_6
    def setGyroOffsetEnabled(enabled):
        reg_write_bits(registers.OFFSET_6, 0x1 if enabled else 0, definitions.GYR_OFFSET_EN, 1) 
    ...
    # @see registers.OFFSET_6
    def getXGyroOffset():
        offset = reg_read(registers.OFFSET_3)
        offset |= (reg_read_bits(registers.OFFSET_6, definitions.GYR_OFFSET_X_MSB_BIT, definitions.GYR_OFFSET_X_MSB_LEN)) << 8
    ...
    # @see registers.OFFSET_3
    # @see registers.OFFSET_6
    def setXGyroOffset(offset):
        reg_write(registers.OFFSET_3, offset)
        reg_write_bits(registers.OFFSET_6, offset >> 8, definitions.GYR_OFFSET_X_MSB_BIT, definitions.GYR_OFFSET_X_MSB_LEN)
    ...
    # @see registers.OFFSET_6
    def getYGyroOffset():
        offset = reg_read(registers.OFFSET_4)
        offset |= (reg_read_bits(registers.OFFSET_6, definitions.GYR_OFFSET_Y_MSB_BIT, definitions.GYR_OFFSET_Y_MSB_LEN)) << 8
        return self._sign_extend(offset, 10)
    ...
    # @see registers.OFFSET_4
    # @see registers.OFFSET_6
    def setYGyroOffset(offset):
        reg_write(registers.OFFSET_4, offset)
        reg_write_bits(registers.OFFSET_6, offset >> 8, definitions.GYR_OFFSET_Y_MSB_BIT, definitions.GYR_OFFSET_Y_MSB_LEN)
    ...
    # @see registers.OFFSET_6
    def getZGyroOffset():
        offset = reg_read(registers.OFFSET_5)
        offset |= (reg_read_bits(registers.OFFSET_6, definitions.GYR_OFFSET_Z_MSB_BIT, definitions.GYR_OFFSET_Z_MSB_LEN)) << 8;
        return self._sign_extend(offset, 10)
    ...
    # @see registers.OFFSET_5
    # @see registers.OFFSET_6
    def setZGyroOffset(offset):
        reg_write(registers.OFFSET_5, offset)
        reg_write_bits(registers.OFFSET_6, offset >> 8, definitions.GYR_OFFSET_Z_MSB_BIT, definitions.GYR_OFFSET_Z_MSB_LEN)
    ...
    def getFreefallDetectionThreshold():
        return reg_read(registers.INT_LOWHIGH_1)

    ## Set free-fall event acceleration threshold.
    # @param threshold New free-fall acceleration threshold value (LSB = 7.81mg, 0 = 3.91mg)
    # @see getFreefallDetectionThreshold()
    # @see registers.INT_LOWHIGH_1
    def setFreefallDetectionThreshold(threshold):
        reg_write(registers.INT_LOWHIGH_1, threshold)
    ...
    def getFreefallDetectionDuration():
        return reg_read(registers.INT_LOWHIGH_0)

    ## Set free-fall event duration threshold.
    # @param duration New free-fall duration threshold value (LSB = 2.5ms, 0 = 2.5ms)
    # @see getFreefallDetectionDuration()
    # @see registers.INT_LOWHIGH_0
    def setFreefallDetectionDuration(duration):
        reg_write(registers.INT_LOWHIGH_0, duration)
    ...
    def getShockDetectionThreshold():
        return reg_read(registers.INT_LOWHIGH_4)
    ...
    # @see registers.INT_LOWHIGH_4
    def setShockDetectionThreshold(threshold):
        reg_write(registers.INT_LOWHIGH_4, threshold)
    ...
    def getShockDetectionDuration():
        return reg_read(registers.INT_LOWHIGH_3)

    ## Set free-fall event duration threshold.
    # @param duration New free-fall duration threshold value (LSB = 2.5ms, 0 = 2.5ms)
    # @see getFreefallDetectionDuration()
    # @see registers.INT_LOWHIGH_3
    def setShockDetectionDuration(duration):
        reg_write(registers.INT_LOWHIGH_3, duration)
    ...
    def getStepDetectionMode():
        ret_step_conf0 = 0
        ret_min_step_buf = 0

        ret_step_conf0 = reg_read(registers.STEP_CONF_0)

        # min_step_buf is the first 3 bits of RA_STEP_CONF_1
        # the 4th bit is the enable bit (step_cnt_en)
        ret_min_step_buf = reg_read_bits(registers.STEP_CONF_1, 0, 3);
    ...
    def setStepDetectionMode(mode):
        ...
        reg_write(registers.STEP_CONF_0, step_conf0);
        reg_write_bits(registers.STEP_CONF_1, min_step_buf, definitions.STEP_BUF_MIN_BIT, definitions.STEP_BUF_MIN_LEN)
    ...
    def getStepCountEnabled():
        return 0 != (reg_read_bits(registers.STEP_CONF_1, definitions.STEP_CNT_EN_BIT, 1))
    ...
    # @see registers.STEP_CONF_1
    # @see definitions.STEP_CNT_EN_BIT
    def setStepCountEnabled(enabled):
        return reg_write_bits(registers.STEP_CONF_1, 0x1 if enabled else 0, definitions.STEP_CNT_EN_BIT, 1)
    ...
    # @see registers.CMD
    def resetStepCount():
        reg_write(registers.CMD, commands.STEP_CNT_CLR)
    ...
    def getMotionDetectionThreshold():
        return reg_read(registers.INT_MOTION_1)

    ## Set motion detection event acceleration threshold.
    # @param threshold New motion detection acceleration threshold value
    # @see getMotionDetectionThreshold()
    # @see registers.INT_MOTION_1
    def setMotionDetectionThreshold(threshold):
        return reg_write(registers.INT_MOTION_1, threshold)
    ...
    def getMotionDetectionDuration():
        return 1 + reg_read_bits(registers.INT_MOTION_0, definitions.ANYMOTION_DUR_BIT, definitions.ANYMOTION_DUR_LEN)

    ## Set motion detection event duration threshold.
    # @param duration New motion detection duration threshold value (#samples [1-4])
    # @see getMotionDetectionDuration()
    # @see registers.INT_MOTION_0
    def setMotionDetectionDuration(samples):
        reg_write_bits(registers.INT_MOTION_0, samples - 1, definitions.ANYMOTION_DUR_BIT, definitions.ANYMOTION_DUR_LEN)
    ...
    def getZeroMotionDetectionThreshold():
        return reg_read(registers.INT_MOTION_2)

    ## Set zero motion detection event acceleration threshold.
    # @param threshold New zero motion detection acceleration threshold value
    # @see getZeroMotionDetectionThreshold()
    # @see registers.INT_MOTION_2
    def setZeroMotionDetectionThreshold(threshold):
        reg_write(registers.INT_MOTION_2, threshold)
    ...
    def getZeroMotionDetectionDuration():
        return reg_read_bits(registers.INT_MOTION_0, definitions.NOMOTION_DUR_BIT, definitions.NOMOTION_DUR_LEN)
    ...
    # @see registers.INT_MOTION_0
    # @see BMI160ZeroMotionDuration
    def setZeroMotionDetectionDuration(duration):
        reg_write_bits(registers.INT_MOTION_0, duration, definitions.NOMOTION_DUR_BIT, definitions.NOMOTION_DUR_LEN)
    ...
    def getTapDetectionThreshold():
        return reg_read_bits(registers.INT_TAP_1, definitions.TAP_THRESH_BIT, definitions.TAP_THRESH_LEN)

    ## Set tap event acceleration threshold.
    # @param threshold New tap acceleration threshold value
    # @see getTapDetectionThreshold()
    # @see registers.INT_TAP_1
    def setTapDetectionThreshold(threshold):
        reg_write_bits(registers.INT_TAP_1, threshold, definitions.TAP_THRESH_BIT, definitions.TAP_THRESH_LEN)
    ...
    def getTapShockDuration():
        return 0 != (reg_read_bits(registers.INT_TAP_0, definitions.TAP_SHOCK_BIT, 1))
    ...
    # @see registers.INT_TAP_0
    def setTapShockDuration(duration):
        reg_write_bits(registers.INT_TAP_0, 0x1 if duration else 0, definitions.TAP_SHOCK_BIT, 1)
    ...
    def getTapQuietDuration():
        return 0 != (reg_read_bits(registers.INT_TAP_0, definitions.TAP_QUIET_BIT, 1))
    ...
    # @see registers.INT_TAP_0
    def setTapQuietDuration(duration):
        reg_write_bits(registers.INT_TAP_0, 0x1 if duration else 0, definitions.TAP_QUIET_BIT, 1)
    ...
    def getDoubleTapDetectionDuration():
        return reg_read_bits(registers.INT_TAP_0, definitions.TAP_DUR_BIT, definitions.TAP_DUR_LEN)
    ...
    # @see registers.INT_TAP_0
    def setDoubleTapDetectionDuration(duration):
        reg_write_bits(registers.INT_TAP_0, duration, definitions.TAP_DUR_BIT, definitions.TAP_DUR_LEN)
    ...
    #*/
    def getIntFreefallEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_1, definitions.LOW_G_EN_BIT, definitions.LOW_G_EN_LEN))
    ...
    # @see definitions.LOW_G_EN_BIT
    #*/
    def setIntFreefallEnabled(enabled):
        reg_write_bits(registers.INT_EN_1, 0x1 if enabled else 0,durationdefinitions.LOW_G_EN_BIT,durationdefinitions.LOW_G_EN_LEN)
    ...
    #*/
    def getIntShockEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_1, definitions.HIGH_G_EN_BIT, definitions.HIGH_G_EN_LEN))
    ...
    # @see definitions.HIGH_G_EN_BIT
    #*/
    def setIntShockEnabled(enabled):
        reg_write_bits(registers.INT_EN_1, 0x7 if enabled else 0x0, definitions.HIGH_G_EN_BIT, definitions.HIGH_G_EN_LEN)
    ...
    #*/
    def getIntStepEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_2, definitions.STEP_EN_BIT, 1))
    ...
    # @see definitions.STEP_EN_BIT
    #*/
    def setIntStepEnabled(enabled):
        reg_write_bits(registers.INT_EN_2, 0x1 if enabled else 0x0, definitions.STEP_EN_BIT, 1)
    ...
    #*/
    def getIntMotionEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_0, definitions.ANYMOTION_EN_BIT, definitions.ANYMOTION_EN_LEN))
    ...
    # @see definitions.ANYMOTION_EN_BIT
    #*/
    def setIntMotionEnabled(enabled):
        # Enable for all 3 axes 
        reg_write_bits(registers.INT_EN_0, 0x7 if enabled else 0x0, definitions.ANYMOTION_EN_BIT, definitions.ANYMOTION_EN_LEN)
    ...
    #*/
    def getIntZeroMotionEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_2, definitions.NOMOTION_EN_BIT, definitions.NOMOTION_EN_LEN))
    ...
    #*/
    def setIntZeroMotionEnabled(enabled):
        if (enabled):
            # Select No-Motion detection mode
            reg_write_bits(registers.INT_MOTION_3, 0x1, definitions.NOMOTION_SEL_BIT, definitions.NOMOTION_SEL_LEN)
        # Enable for all 3 axes
        reg_write_bits(registers.INT_EN_2, 0x7 if enabled else 0x0, definitions.NOMOTION_EN_BIT, definitions.NOMOTION_EN_LEN)
    ...
    #*/
    def getIntTapEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_0, definitions.S_TAP_EN_BIT, 1))
    ...
    # @see definitions.S_TAP_EN_BIT
    #*/
    def setIntTapEnabled(enabled):
        reg_write_bits(registers.INT_EN_0, 0x1 if enabled else 0, definitions.S_TAP_EN_BIT, 1)
    ...
    #*/
    def getIntDoubleTapEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_0, definitions.D_TAP_EN_BIT, 1))
    ...
    # @see definitions.D_TAP_EN_BIT
    #*/
    def setIntDoubleTapEnabled(enabled):
        reg_write_bits(registers.INT_EN_0, 0x1 if enabled else 0, definitions.D_TAP_EN_BIT, 1)
    ...
    #*/
    def getIntFIFOBufferFullEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_1, definitions.FFULL_EN_BIT, 1))
    ...
    # @see definitions.FFULL_EN_BIT
    #*/
    def setIntFIFOBufferFullEnabled(enabled):
        reg_write_bits(registers.INT_EN_1, 0x1 if enabled else 0x0, definitions.FFULL_EN_BIT, 1)
    ...
    def getIntDataReadyEnabled():
        return 0 != (reg_read_bits(registers.INT_EN_1, definitions.DRDY_EN_BIT, 1))
    ...
    # @see registers.INT_EN_1
    # @see definitions.DRDY_EN_BIT
    def setIntDataReadyEnabled(enabled):
        reg_write_bits(registers.INT_EN_1, 0x1 if enabled else 0x0, definitions.DRDY_EN_BIT, 1)
    ...
    def getAccelFIFOEnabled():
        return 0 != (reg_read_bits(registers.FIFO_CONFIG_1, definitions.FIFO_ACC_EN_BIT, 1))

    ## Set accelerometer FIFO enabled value.
    # @param enabled New accelerometer FIFO enabled value
    # @see getAccelFIFOEnabled()
    # @see registers.FIFO_CONFIG_1
    def setAccelFIFOEnabled(enabled):
        reg_write_bits(registers.FIFO_CONFIG_1, 0x1 if enabled else 0, definitions.FIFO_ACC_EN_BIT, 1)
    ...
    # @see registers.FIFO_CONFIG_1
    def getGyroFIFOEnabled():
        return 0 != (reg_read_bits(registers.FIFO_CONFIG_1, definitions.FIFO_GYR_EN_BIT, 1))

    ## Set gyroscope FIFO enabled value.
    # @param enabled New gyroscope FIFO enabled value
    # @see getGyroFIFOEnabled()
    # @see registers.FIFO_CONFIG_1
    def setGyroFIFOEnabled(enabled):
        reg_write_bits(registers.FIFO_CONFIG_1, 0x1 if enabled else 0, definitions.FIFO_GYR_EN_BIT, 1)
    ...
    def resetFIFO():
        reg_write(registers.CMD, commands.FIFO_FLUSH)
    ...
    # @see commands.FIFO_FLUSH
    def resetInterrupt():
        reg_write(registers.CMD, commands.INT_RESET)
    ...
    def getFIFOHeaderModeEnabled():
        return 0 != (reg_read_bits(registers.FIFO_CONFIG_1, definitions.FIFO_HEADER_EN_BIT, 1))
    ...
    # @see registers.FIFO_CONFIG_1
    # @see definitions.FIFO_HEADER_EN_BIT
    def setFIFOHeaderModeEnabled(enabled):
        reg_write_bits(registers.FIFO_CONFIG_1, 0x1 if enabled else 0, definitions.FIFO_HEADER_EN_BIT, 1)
    ...
    def getIntStatus0():
        return reg_read(registers.INT_STATUS_0)
    ...
    def getIntStatus1():
        return reg_read(registers.INT_STATUS_1)
    ...
    def getIntStatus2():
        return reg_read(registers.INT_STATUS_2)
    ...
    def getIntStatus3():
        return reg_read(registers.INT_STATUS_3)
    ...
    def getIntFreefallStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_1, definitions.LOW_G_INT_BIT, 1))
    ...
    def getIntTapStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_0, definitions.S_TAP_INT_BIT, 1))
    ...
    def getIntDoubleTapStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_0, definitions.D_TAP_INT_BIT, 1))
    ...
    def getIntShockStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_1, definitions.HIGH_G_INT_BIT, 1))
    ...
    def getXNegShockDetected():
        status = reg_read(registers.INT_STATUS_3)
    ...
    def getIntStepStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_0, definitions.STEP_INT_BIT, 1))
    ...
    def getIntMotionStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_0, definitions.ANYMOTION_INT_BIT, 1))
    ...
    def getXNegMotionDetected():
        status = reg_read(registers.INT_STATUS_2)
    ...
    def getIntZeroMotionStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_1, definitions.NOMOTION_INT_BIT, 1))
    ...
    def getIntFIFOBufferFullStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_1, definitions.FFULL_INT_BIT, 1))
    ...
    # @see registers.INT_STATUS_1
    # @see definitions.FFULL_INT_BIT
    def getIntDataReadyStatus():
        return 0 != (reg_read_bits(registers.INT_STATUS_1, definitions.DRDY_INT_BIT, 1))
    ...
    def getInterruptMode():
        return not (reg_read_bits(registers.INT_OUT_CTRL, definitions.INT1_LVL, 1))
    ...
    # @see registers.INT_OUT_CTRL
    # @see definitions.INT1_LVL
    def setInterruptMode(mode):
        reg_write_bits(registers.INT_OUT_CTRL, 0x0 if mode else 0x1, definitions.INT1_LVL,1)
    ...
    def getInterruptDrive():
        return 0 != (reg_read_bits(registers.INT_OUT_CTRL, definitions.INT1_OD, 1))
    ...
    def setInterruptDrive(drive):
        reg_write_bits(registers.INT_OUT_CTRL, 0x1 if drive else 0x0, definitions.INT1_OD, 1)
    ...
    def getInterruptLatch():
        return reg_read_bits(registers.INT_LATCH, definitions.LATCH_MODE_BIT, definitions.LATCH_MODE_LEN)
    ...
    # @see registers.INT_LATCH
    # @see BMI160InterruptLatchMode
    def setInterruptLatch(mode):
        reg_write_bits(registers.INT_LATCH, mode, definitions.LATCH_MODE_BIT, definitions.LATCH_MODE_LEN)
    ...
    #*/
    def getIntEnabled():
        return 0 != (reg_read_bits(registers.INT_OUT_CTRL, definitions.INT1_OUTPUT_EN, 1))
    ...
    # @see definitions.INT1_OUTPUT_EN
    #*/
    def setIntEnabled(enabled):
        reg_write_bits(registers.INT_OUT_CTRL, 0x1 if enabled else 0, definitions.INT1_OUTPUT_EN, 1)
    ...
    def getAccelerationY():
        buffer = [0]*2
        buffer[0] = registers.ACCEL_Y_L
        serial_buffer_transfer(buffer, 1, 2)
        return ((buffer[1]) << 8) | buffer[0]
    ...
    def getAccelerationZ():
        buffer = [0]*2
        buffer[0] = registers.ACCEL_Z_L
        serial_buffer_transfer(buffer, 1, 2)
        return ((buffer[1]) << 8) | buffer[0]
    ...
    # @see getMotion6()
    # @see registers.GYRO_Z_L
    def getRotationZ():
        buffer = [0]*2
        buffer[0] = registers.GYRO_Z_L
        serial_buffer_transfer(buffer, 1, 2)
        return ((buffer[1]) << 8) | buffer[0]
